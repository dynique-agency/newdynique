...
export default function Home() {
  ...
  const handleWhatsAppSubmit = (e: React.FormEvent) => {
    ...
    // Zelfde patroon als /gratis-checklist: een korte vertraging zodat de WhatsApp-tab
    // gegarandeerd opent vóór we wegnavigeren, dan door naar de bedankpagina als zichtbare
    // bevestiging — voorheen bleef de pagina na versturen zonder enig teken van succes staan.
    setSubmitting(true);
    setTimeout(() => {
      window.location.href = `/bedankt?name=${encodeURIComponent(formData.name)}`;
    }, 600);
  };
  ...
  return (
    <>
        ...
        <div className={`fixed inset-0 z-[9999] bg-[#070707] flex items-center justify-center transition-all duration-1000 ${fadeOut ? "opacity-0 scale-[1.04]" : "opacity-100 scale-100"}`}>
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <div className="overflow-hidden">
              <div className="load-title text-6xl sm:text-7xl md:text-9xl font-extralight text-white tracking-[0.35em]" aria-hidden="true">
                DYNIQUE
              </div>
            </div>
      ...
      <main className="relative">
        ...
        {/* ─── 9. CONTACT ──────────────────────────────────────── */}
        <section className="bg-white py-32 lg:py-40 scroll-mt-24" id="contact">
          <div className="container mx-auto px-6 lg:px-12">
            <div className="grid lg:grid-cols-2 gap-16 lg:gap-32">
              ...
              <div className="space-y-12">
                <div>
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 anim delay-1">
                    <span className="flex items-center gap-1.5 text-black/50 text-xs font-light tracking-wide">
                      <svg className="w-3.5 h-3.5 text-[#d4a574]" fill="currentColor" viewBox="0 0 20 20"><path d="M10 1l2.6 5.6 6.1.8-4.5 4.2 1.2 6-5.4-3-5.4 3 1.2-6L1.3 7.4l6.1-.8L10 1z"/></svg>
                      5.0 · geverifieerde reviews
                    </span>
                    <span className="w-1 h-1 rounded-full bg-black/15"></span>
                    <span className="text-black/50 text-xs font-light tracking-wide">30+ klanten</span>
                  </div>
                </div>
                <div className="space-y-6 anim delay-2">
                  ...
                  <div className="space-y-4 pt-4 border-t border-black/5">
                    ...
                    {[
                      ...
                      { label: "WERKGEBIED", value: "Heel Nederland & België — op locatie in Zuid-Limburg, digitale intake voor de rest", href: null },
                    ].map((item) => (
              ...
              <form onSubmit={handleWhatsAppSubmit} className="space-y-8 anim delay-2">
                <div className="grid md:grid-cols-2 gap-6">
                  {[
                    { label: "Naam *", key: "name", placeholder: "Jouw naam", type: "text", required: true, autoComplete: "name" },
                    { label: "E-mail *", key: "email", placeholder: "naam@bedrijf.nl", type: "email", required: true, autoComplete: "email" },
                    { label: "Bedrijf", key: "company", placeholder: "Optioneel", type: "text", required: false, autoComplete: "organization" },
                  ].map((f) => (
                ...
                {wantsAppointment && (
                  <div className="grid md:grid-cols-3 gap-6 pt-2">
                    ...
                    {formData.meetingType === "fysiek" && (
                      <p className="md:col-span-3 text-black/45 text-xs font-light leading-relaxed tracking-wide -mt-2">
                        We komen graag langs in Zuid-Limburg en de Euregio. Zit je verder weg? Dan starten we met een digitale intake en plannen we een bezoek zodra dat voor het project zinvol is — bespreken we gewoon even.
                      </p>
                    )}
                  </div>
                )}
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="group w-full mt-4 px-12 py-4 bg-black text-white text-xs tracking-[0.25em] font-light hover:bg-[#25D366] transition-all duration-500 flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:bg-black"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                  </svg>
                  {submitting ? "EVEN GEDULD…" : "VERSTUUR VIA WHATSAPP"}
                </button>
              </form>
            </div>
          </div>
        ...
        {/* ─── 10. FAQ ──────────────────────────────────────────── */}
        <section className="bg-zinc-950 py-32 lg:py-40 scroll-mt-24" id="faq">
          <div className="container mx-auto px-6 lg:px-12">
