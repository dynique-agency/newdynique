...
export default function ContactPage() {
  ...
  const inputCls = "w-full bg-black/40 border border-white/10 text-white/90 text-sm font-light tracking-wide px-4 py-3 rounded outline-none focus:border-white/30 transition-colors duration-300 placeholder:text-white/25";
                ...
                <div className="hv hv-3 space-y-4 pt-8 border-t border-white/[0.08]">
                  {DETAILS.map((d) => (
                    <div key={d.label} className="flex items-baseline gap-4">
                      <span className="text-white/25 text-[9px] tracking-[0.35em] font-light flex-shrink-0 w-28">{d.label}</span>
                      {d.href ? (
              ...
              <div className="lg:col-span-7 hv hv-4">
                <form onSubmit={submit}
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-white/40 text-[10px] tracking-[0.3em] font-light mb-2.5">NAAM *</label>
                      <input type="text" required value={form.name} onChange={set("name")} placeholder="Jouw naam" className={inputCls} />
                    </div>
                    <div>
                      <label className="block text-white/40 text-[10px] tracking-[0.3em] font-light mb-2.5">BEDRIJF</label>
                      <input type="text" value={form.company} onChange={set("company")} placeholder="Optioneel" className={inputCls} />
                    </div>
                    <div>
                      <label className="block text-white/40 text-[10px] tracking-[0.3em] font-light mb-2.5">TELEFOON</label>
                      <input type="tel" value={form.phone} onChange={set("phone")} placeholder="06 ..." className={inputCls} />
                    </div>
                    <div>
                      <label className="block text-white/40 text-[10px] tracking-[0.3em] font-light mb-2.5">E-MAIL</label>
                      <input type="email" value={form.email} onChange={set("email")} placeholder="naam@bedrijf.nl" className={inputCls} />
                    </div>
                  </div>

                  <div className="mt-5">
                    <label className="block text-white/40 text-[10px] tracking-[0.3em] font-light mb-2.5">WAT HEB JE NODIG? *</label>
                  ...
                  <div className="mt-5">
                    <label className="block text-white/40 text-[10px] tracking-[0.3em] font-light mb-2.5">OMSCHRIJF JE PROJECT *</label>
                  ...
                  <div className="mt-6 pt-6 border-t border-white/[0.08]">
                    ...
                    {wantsCall && (
                      <div className="grid sm:grid-cols-3 gap-4 mt-5">
                        <div>
                          <label className="block text-white/40 text-[10px] tracking-[0.3em] font-light mb-2.5">DATUM</label>
                          <input type="date" value={appt.date} onChange={(e) => setAppt({ ...appt, date: e.target.value })} className={inputCls} />
                        </div>
                        <div>
                          <label className="block text-white/40 text-[10px] tracking-[0.3em] font-light mb-2.5">TIJD</label>
                          <select value={appt.time} onChange={(e) => setAppt({ ...appt, time: e.target.value })} className={`${inputCls} cursor-pointer appearance-none`}>
                            {["09:00","10:00","11:00","13:00","14:00","15:00","16:00","17:00"].map((t) => <option key={t} value={t} className="bg-[#0c0c0c]">{t}</option>)}
                          </select>
                        </div>
                        <div>
                          <label className="block text-white/40 text-[10px] tracking-[0.3em] font-light mb-2.5">TYPE</label>
                          <select value={appt.type} onChange={(e) => setAppt({ ...appt, type: e.target.value })} className={`${inputCls} cursor-pointer appearance-none`}>
                  ...
                  <p className="mt-4 text-center text-white/30 text-xs font-light tracking-wide">
