...
| 5 | "Maatwerk software limburg" verstevigen — uitzoeken welke pagina hiervoor rankt (positie 12,7, bijna pagina 1) en interne links/diepgang toevoegen | ✅ gedaan (2026-09-08, derde sessie) — `/diensten/processen/limburg` bleek al twee sessies lang **zonder één interne link** te staan (alleen sitemap + eigen layout). Nu gelinkt vanuit Maastricht/Heerlen/Sittard (de 3 steden die de pagina zelf noemt) via een nieuw `relatedRegionLink`-veld, én vanuit `diensten/processen`'s "VERDER LEZEN" |
...
| 8 | Kritische zelf-audit van alle 9(→10) locatiepagina's, op verzoek van de eigenaar — zie `docs/PROJECT-LOG.md` voor de volledige bevindingenlijst (JSON-LD nog "Webdesign" i.p.v. "Maatwerk software"; "Cross-border Business"-highlight + btw-FAQ woord-voor-woord gedupliceerd op 4-5 pagina's; CTA's linkten naar het generieke homepage-formulier i.p.v. een eigen leadformulier; geen social proof; geen prijs-FAQ; geen `/locaties`-hub-pagina) | ✅ alle bevindingen opgelost (2026-09-08, derde sessie) |

**Volgende stap:** alle items uit dit plan zijn afgerond. Vervolgstap is meten — Search Console laten lopen op alle nieuwe/gewijzigde URL's voordat er weer nieuwe pagina's bijkomen (zie `STRATEGY.md` §5's eigen "meet voor je verder bouwt"-discipline, die deze sessie zelf niet gevolgd is — zie `docs/PROJECT-LOG.md`).
